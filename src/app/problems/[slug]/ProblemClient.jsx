...
export default function ProblemClient({ slug }) {
  ...
  const getOutputDetails = () => {
    ...
    if (output.status.id === 3) {
      return <pre className="p-2 bg-green-900 text-white rounded mt-4 whitespace-pre-wrap">{output.stdout || "No output"}</pre>;
    }
    if (output.status.id === 6) {
      return <pre className="p-2 bg-red-900 text-white rounded mt-4 whitespace-pre-wrap">{output.compile_output}</pre>;
    }
    return <pre className="p-2 bg-red-900 text-white rounded mt-4 whitespace-pre-wrap">{output.stderr || "An error occurred."}</pre>;
  };
